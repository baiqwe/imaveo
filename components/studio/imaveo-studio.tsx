...
const promptExamples: Record<StudioMode, { en: string; zh: string }[]> = {
  "text-to-image": [
    { en: "luxury skincare product hero shot, editorial lighting, soft gold accents", zh: "高端护肤品主视觉，杂志感灯光，带柔和金色点缀" },
  ...
  "text-to-video": [
    ...
    { en: "fashion editorial scene, model turns toward camera, soft breeze, golden hour", zh: "时尚 editorial 场景，模特转向镜头，微风，黄金时刻" },
  ],
  "image-to-video": [
    { en: "subtle camera drift, character blink, hair motion, cinematic depth", zh: "轻微镜头漂移，角色眨眼，头发摆动，电影感景深" },
    { en: "animate the poster with parallax depth and slow reveal motion", zh: "让海报产生视差景深与缓慢揭示的运动" },
  ],
};
      ...
      <div className="panel-lift rounded-[24px] border border-zinc-800 bg-black/25 p-4">
        ...
        <div className="mt-4 space-y-3">
          {strengths.map((strength) => (
            <div key={strength} className="flex items-start gap-3 text-sm leading-6 text-zinc-300">
...
function ChipRow({ locale, groups }: { locale: string; groups: GeneratorChipGroup[] }) {
  const isZh = locale === "zh";
...
function GeneratorPanel({
  ...
  const isZh = locale === "zh";
  const currentMode = getModeConfig(mode);
  const examples = promptExamples[mode];
  ...
  const chipGroups: GeneratorChipGroup[] = [
    ...
    {
      key: "duration",
      label: { en: mode.includes("video") ? "Duration" : "Style Intent", zh: mode.includes("video") ? "时长" : "风格倾向" },
    ...
    {
      key: "quantity",
      label: { en: mode.includes("video") ? "Output Count" : "Variants", zh: mode.includes("video") ? "输出数量" : "输出变体" },
    ...
    <div className="aurora-stage rounded-[34px] border border-zinc-800 bg-[linear-gradient(180deg,rgba(47,32,22,0.72),rgba(20,14,11,0.95))] p-5 shadow-[0_32px_90px_rgba(0,0,0,0.34)] md:p-7">
      ...
      <div className="mt-8 rounded-[30px] border border-[#3e3026] bg-[#32261f]/95 p-6 md:p-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-[11px] uppercase tracking-[0.22em] text-zinc-500">{isZh ? "Prompt" : "Prompt"}</div>
          <button
            type="button"
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[180px_minmax(0,1fr)]">
            ...
            <div className="panel-lift rounded-[24px] border border-[#4c392b] bg-[#2a1f18]/80 p-4">
              {sourcePreview ? (
                ...
                >
                  ...
                  <div className="mt-3 text-sm font-medium text-white">{isZh ? "上传参考图" : "Upload source"}</div>
                  <div className="mt-1 text-xs leading-5 text-zinc-400">{isZh ? "图生图 / 图生视频可选" : "Optional for image-based generation"}</div>
                </button>
              )}
            </div>
          ) : null}
          ...
          <Textarea
            ...
            placeholder={currentMode.promptPlaceholder[isZh ? "zh" : "en"]}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="panel-lift rounded-[24px] border border-[#3e3026] bg-[#2a1f18]/80 p-4">
            ...
            <Select value={selectedModelId} onValueChange={onModelChange}>
              <SelectTrigger className="h-16 rounded-[20px] border-zinc-700 bg-[#1a130f] px-4 text-left text-white focus:ring-primary/35 focus:ring-offset-0">
                <SelectValue placeholder={isZh ? "选择模型" : "Choose a model"} />
              </SelectTrigger>
              <SelectContent className="border-zinc-800 bg-[#14100d] text-zinc-100">
                {availableModels.map((model) => (
                  <SelectItem key={model.slug} value={model.slug} className="focus:bg-white/[0.06] focus:text-white">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            <button type="button" onClick={() => setPrompt("")} className="text-sm text-zinc-400 transition-colors hover:text-white">
              {isZh ? "清空" : "Clear"}
            </button>
            <Button className="h-14 rounded-full px-8 text-base shadow-[0_0_36px_rgba(245,197,24,0.22)]">
              {liveGeneration ? (isZh ? "继续进入真实编辑器" : "Continue to live editor") : isZh ? "生成" : "Generate"}
        ...
        <div className="space-y-4">
          ...
          <div className="panel-lift rounded-[24px] border border-zinc-800 bg-[radial-gradient(circle_at_top,rgba(245,197,24,0.12),rgba(0,0,0,0.92)_65%)] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm leading-7 text-zinc-300">
              <div className="flex items-start gap-3">
                <CheckCircle2 className="mt-1 h-4 w-4 flex-none text-primary" />
                <span>{isZh ? "如果是视频模式，先保存当前创意和模型组合。" : "If you are in a video mode, preserve this prompt and model combination first."}</span>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="mt-1 h-4 w-4 flex-none text-primary" />
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <Button asChild className="rounded-full px-5">
                <Link href={`/${locale}/pricing`}>{isZh ? "查看套餐与 Credits" : "Compare plans and credits"}</Link>
              </Button>
              <Button asChild variant="outline" className="rounded-full border-zinc-700 bg-transparent text-zinc-200 hover:bg-white/[0.04]">
                <Link href={`/${locale}/${mode.includes("video") ? "ai-video" : "ai-image"}`}>{isZh ? "查看模型文档" : "Browse model docs"}</Link>
              </Button>
            </div>
          </div>
        </div>
...
export function ImaveoStudio({ locale, initialMode, initialModel, initialStyle }: ImaveoStudioProps) {
  const isZh = locale === "zh";
      ...
      <div className="container px-4 md:px-6">
        <div className="mx-auto max-w-[1440px]">
            ...
            <div className="flex items-center gap-3">
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="outline" className="rounded-full border-zinc-700 bg-transparent text-zinc-100 hover:bg-white/[0.04] xl:hidden">
          ...
          <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)_280px]">
            <aside className="hidden xl:block xl:sticky xl:top-24 xl:self-start">{mobileSidebar}</aside>
                    ...
                    <div key={item.id} className="panel-lift rounded-[22px] border border-zinc-800 bg-black/30 p-3">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="text-xs uppercase tracking-[0.18em] text-zinc-500">{item.status}</div>
                        </div>
                        <Button variant="ghost" size="sm" className="rounded-full text-zinc-300 hover:bg-white/[0.04] hover:text-white">
                          {isZh ? "复用" : "Reuse"}
                        </Button>
                      </div>
                    </div>
